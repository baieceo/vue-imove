<template>
  <div v-loading="loading">
    <EditorHeader v-if="!hideEditorHeader" default-active="4">
      <el-button @click="handleImportSchema">导入Schema</el-button>
      <el-button plain @click="handleToPlayground">
        Playground中验证
      </el-button>
      <el-button type="primary" plain @click="handlePreview">
        预览展示
      </el-button>
      <el-button type="primary" @click="handleExportSchema">
        导出Schema
      </el-button>
    </EditorHeader>
    ...
    <div :class="[$style.container, containerClass]">
      <div :class="{
        ...
        <div :class="[$style.contentBox, contentBoxClass]">
          <slot name="content-toolbar" />
          <div :class="[contentFormClass]">
        ...
        <div :class="[$style.rightForm, rightFormClass]">
    ...
    methods: {
      ...
      getExportCode({
        ...
        return {
          ...
          uiSchema: {},
          formFooter: designer ? {
            ...defaultConfig.formFooter,
            ...formFooter
          } : filter(formFooter, defaultConfig.formFooter),
          ...
          if (errorNode.length > 0 && Array.isArray(errorNode)) {
            return this.$msgbox({
              ...
              message: this.$createElement(
                ...
                errorNode.map((item) =>
                  this.$createElement('pre', null, JSON.stringify(item, null, 4))
                )
              ),
            });
          }
      ...
      handleToPlayground(isUrl) {
        const codeObj = this.getExportCode();
        const schemas = {};

        const urlQueryString = Object.keys(codeObj)
          .reduce((pre, cur) => {
            schemas[cur] = encodeURIComponent(JSON.stringify(codeObj[cur]));

            pre.push(
              `${cur}=${encodeURIComponent(JSON.stringify(codeObj[cur]))}`
            );
            return pre;
          }, [])
          .join('&');
        ...
        if (isUrl) {
          const link = `/schema/playground?type=Test&${urlQueryString}`;
